...
8. 基于当前轮 MCP 数据完成 V2 要求的市场分析；把排名、排除项和 seed list 写入 `candidate.ranked` / `candidate.filtered`。
...
14. 输出强制最终每轮总结，写入 `summary.finalized` 审计事件。无论是不交易、阻塞、dry-run、错误还是超时轮次，都必须输出。
15. 按配置间隔等待，并重复循环，同时动态管理所有已有持仓。
