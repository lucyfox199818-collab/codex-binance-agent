...
当用户明确提到 `$trading-v1`、`trading-v1`、`V1.txt`，或要求按 V1 运行交易流程时：

1. 先读取 `.cursor/skills/trading-v1/SKILL.md`。
2. 把该文件视为项目本地 skill 正文并遵循其说明。
3. 相对引用、脚本、资产和 agent 元数据都从 `.cursor/skills/trading-v1/` 解析。
4. 不要依赖、创建或更新 `~/.codex/skills/trading-v1` 下的用户全局副本。
5. 如果项目本地 skill 文件缺失或不可读，明确说明并在执行任何 V1 交易流程前停止。
